...
class Environment:
    ...
    def get_init_state(self):
        ...
        return State(self, self.robot_init_posit, self.robot_init_orient,
                     self.widget_init_posits, self.widget_init_orients,
                     self.force_valid)

    def perform_action(self, state, action):
        """
        Perform the given action on the given state, and return whether the
        action was successful (i.e. valid and collision free), the cost of
        performing the action, and the resulting new state.
        :param state: 
        :param action: 
    ...
    def is_solved(self, state):
        """
        Check if the environment has been solved (i.e. all target cells are
        covered by a widget)
